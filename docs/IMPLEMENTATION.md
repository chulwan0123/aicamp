...
- 입력: 1/8 실제 만 나이, 2/8 주택 수·주택별 명의, 3/8 주소·동호수, 4/8 공시가격·전용면적·현재 예상 매매가격, 5/8 주택별 취득연도·취득가·거주기간, 6/8 희망 지역·새 집 예산·임차보증금·의료비·부분임대 조건, 7/8 성향·실제 소득·필요 생활비, 8/8 최종 확인
...
- 다운사이징은 주택연금 공시가격 상한을 새 집 매매가격으로 사용하지 않고, 사용자가 입력한 새 집 예상 매매가격과 공시가격을 각각 사용합니다.
...
- 현재 `npm run check`는 78개 테스트가 통과합니다.
